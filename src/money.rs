use std::{
    fmt::Display,
    marker::PhantomData,
    ops::{Add, Div, Mul, Sub},
};

use crate::currency::Currency;

use num::traits::NumOps;

#[derive(Debug)]
pub struct Money<N, C>
where
    N: NumOps + PartialOrd,
    C: Currency,
{
    amount: N,
    currency: PhantomData<C>,
}

impl<N, C> Money<N, C>
where
    N: NumOps + PartialOrd,
    C: Currency,
{
    #[must_use]
    pub const fn new(amount: N) -> Self {
        Self {
            amount,
            currency: PhantomData,
        }
    }

    #[must_use]
    pub fn get_currency_name(self) -> &'static str {
        C::get_name()
    }

    #[must_use]
    pub fn get_currency_symbol(self) -> &'static str {
        C::get_symbol()
    }

    #[must_use]
    pub fn get_currency_alphabetic_code() -> &'static str {
        C::get_alphabetic_code()
    }

    #[must_use]
    pub fn get_currency_numeric_code() -> &'static str {
        C::get_numeric_code()
    }

    #[must_use]
    pub fn get_currency_minor() -> usize {
        C::get_minor()
    }

    #[must_use]
    pub fn get_currency_fractions() -> u16 {
        C::get_fractions()
    }
}

impl<N, C> Display for Money<N, C>
where
    N: NumOps + PartialOrd + Display,
    C: Currency,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // TODO: Add rounding method (according to ISO standards).
        write!(
            f,
            "{} {:.minor$}",
            C::get_symbol(),
            self.amount,
            minor = C::get_minor()
        )
    }
}

impl<N, C, R> PartialEq<Money<R, C>> for Money<N, C>
where
    C: Currency,
    N: NumOps + PartialOrd + From<R>,
    R: NumOps + PartialOrd + Clone,
{
    fn eq(&self, other: &Money<R, C>) -> bool {
        self.amount == other.amount.clone().into()
    }
}

impl<N, C, R> PartialOrd<Money<R, C>> for Money<N, C>
where
    C: Currency,
    N: NumOps + PartialOrd + From<R>,
    R: NumOps + PartialOrd + Clone,
{
    fn partial_cmp(&self, other: &Money<R, C>) -> Option<std::cmp::Ordering> {
        self.amount.partial_cmp(&(other.amount.clone().into()))
    }
}

impl<N, C, R> Add<Money<R, C>> for Money<N, C>
where
    C: Currency,
    N: NumOps + PartialOrd + From<R>,
    R: NumOps + PartialOrd,
{
    type Output = Self;

    fn add(self, rhs: Money<R, C>) -> Self::Output {
        Money::new(self.amount + rhs.amount.into())
    }
}

impl<N, C, R> Sub<Money<R, C>> for Money<N, C>
where
    C: Currency,
    N: NumOps + PartialOrd + From<R>,
    R: NumOps + PartialOrd,
{
    type Output = Self;

    fn sub(self, rhs: Money<R, C>) -> Self::Output {
        Money::new(self.amount - rhs.amount.into())
    }
}

impl<N, C, R> Mul<R> for Money<N, C>
where
    C: Currency,
    N: NumOps + PartialOrd + From<R>,
    R: NumOps<N> + PartialOrd,
{
    type Output = Self;

    fn mul(self, rhs: R) -> Self::Output {
        Money::new(self.amount * rhs.into())
    }
}

impl<N, C, R> Div<Money<R, C>> for Money<N, C>
where
    C: Currency,
    N: NumOps + PartialOrd + From<R>,
    R: NumOps + PartialOrd,
{
    type Output = N;

    fn div(self, rhs: Money<R, C>) -> Self::Output {
        self.amount / rhs.amount.into()
    }
}

impl<N, C> Div<N> for Money<N, C>
where
    C: Currency,
    N: NumOps + PartialOrd,
{
    type Output = Self;

    fn div(self, rhs: N) -> Self::Output {
        Money::new(self.amount / rhs)
    }
}

#[cfg(test)]
mod tests {
    use crate::{currency::USD, money::Money};

    #[cfg(test)]
    mod arithmetic_operations {
        use crate::{currency::USD, money::Money};

        use approx::assert_relative_eq;

        #[test]
        fn test_money_add() {
            let m1: Money<f64, USD> = Money::new(5.0);
            let m2 = Money::new(6.3);
            assert_relative_eq!((m1 + m2).amount, 11.3);
        }

        #[test]
        fn test_money_sub() {
            let m1: Money<f64, USD> = Money::new(12.94);
            let m2 = Money::new(6.3);
            assert_relative_eq!((m1 - m2).amount, 6.64);
        }

        #[test]
        fn test_money_mul_int() {
            let m: Money<f64, USD> = Money::new(5.0);
            assert_relative_eq!((m * 4.0).amount, 20.0);
        }

        #[test]
        fn test_money_mul_float() {
            let m: Money<f64, USD> = Money::new(5.0);
            assert_relative_eq!((m * 1.5).amount, 7.5);
        }

        #[test]
        fn test_money_div_money() {
            let m1: Money<f64, USD> = Money::new(6.0);
            let m2: Money<f64, USD> = Money::new(6.0);
            assert_relative_eq!(m1 / m2, 1.0);
        }

        #[test]
        fn test_money_div_int() {
            let m: Money<f64, USD> = Money::new(6.0);
            assert_relative_eq!((m / 3.0).amount, 2.0);
        }

        #[test]
        fn test_money_div_float() {
            let m: Money<f64, USD> = Money::new(6.0);
            assert_relative_eq!((m / 3.0).amount, 2.0);
        }
    }
    #[test]
    fn test_money_display() {
        let m: Money<f64, USD> = Money::new(4.32123);
        assert_eq!(m.to_string(), "$ 4.32");
    }

    #[test]
    fn test_money_partial_eq() {
        let m1: Money<f64, USD> = Money::new(157.34);
        let m2: Money<i32, USD> = Money::new(3);
        assert_ne!(m1, m2);
        assert_eq!(m1, m1);
    }

    #[test]
    fn test_money_partial_ord() {
        let m1: Money<f64, USD> = Money::new(41.34);
        let m2: Money<f64, USD> = Money::new(324.3);
        assert!(m2 > m1);
        assert!(m2 >= m1);
        assert!(m1 < m2);
        assert!(m1 <= m2);
    }
}
