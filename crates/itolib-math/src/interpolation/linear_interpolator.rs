    ...
    V: FloatScalable,
{
    ...
    pub fn new_from_points(points: Vec<(I, V)>) -> Self {
        let mut ret = Self { points: BTreeMap::new() };
...
mod tests {
    ...
    fn test_linear_interpolator() {
        ...
        assert_eq!(interpolator.interpolate(OrderedFloat(1.0)), InterpolationResult::NoPoints);
        ...
        assert!(interpolator.add_point((OrderedFloat(4.0), OrderedFloat(40.0))).is_none());
        assert_eq!(interpolator.points.len(), 4);

        assert!(interpolator.add_point((OrderedFloat(4.0), OrderedFloat(50.0))).is_some());
        assert_eq!(interpolator.points.len(), 4);

        assert_eq!(interpolator.range().unwrap(), (OrderedFloat(1.0), OrderedFloat(4.0)));
        ...
        assert_eq!(interpolator.interpolate(OrderedFloat(6.0)), InterpolationResult::OutOfRange);

        // Test remove points.
        let _ = interpolator.remove_points([OrderedFloat(4.0), OrderedFloat(1.0)].to_vec());
        assert_eq!(interpolator.interpolate(OrderedFloat(1.0)), InterpolationResult::OutOfRange);
    }
}
