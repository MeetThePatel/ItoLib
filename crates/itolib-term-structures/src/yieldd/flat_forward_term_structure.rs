    ...
    D: DayCounter,
{
    ...
    fn zero_rate(&self, t: DateTime) -> Result<InterestRate<C, D>, TermStructureError> {
        ...
        let compound_factor = 1.0 / self.discount_factor(t)?.value();

        let compounding = self.rate.get_compounding();

        Ok(implied_rate_from_compound_factor(
            CompoundFactor::new(compound_factor).unwrap(),
            &day_count_fraction,
    ...
    fn forward_rate(
        ...
        let compound_factor = self.discount_factor(t1)?.value() / self.discount_factor(t2)?.value();
...
mod tests {
    ...
    fn test_flat_forward_term_structure_discount_factor() {
        ...
        assert_approx_eq!(term_structure.discount_factor(ref_date).unwrap().value(), 1.0, 10e-8);
        assert_approx_eq!(
            term_structure.discount_factor(ref_date_plus_1_year).unwrap().value(),
            0.955_879_62,
            10e-8
        );
        assert_approx_eq!(
            term_structure.discount_factor(ref_date_plus_1_year_7_month).unwrap().value(),
            0.931_219_48,
            10e-8
        );
    }
    ...
    fn test_flat_forward_term_structure_zero_rate() {
        ...
        assert_approx_eq!(
            term_structure.zero_rate(ref_date_plus_1_year).unwrap().get_rate(),
            flat_rate.get_rate(),
            10e-8
        );
        assert_approx_eq!(
            term_structure.zero_rate(ref_date_plus_1_year_7_month).unwrap().get_rate(),
            flat_rate.get_rate(),
            10e-8
        );
    }
    ...
    fn test_flat_forward_term_structure_forward_rate() {
        ...
        // Test forward rate
        assert_approx_eq!(
            term_structure.forward_rate(ref_date, ref_date_plus_1_year).unwrap().get_rate(),
            flat_rate.get_rate(),
            10e-8
        );
        assert_approx_eq!(
        ...
        assert_approx_eq!(
            term_structure.forward_rate(ref_date, ref_date_plus_1_year_7_month).unwrap().get_rate(),
            flat_rate.get_rate(),
            10e-8
        );
    }
