...
impl<'a> ExchangeRateManager<'a> {
    ...
    pub const fn new() -> Self {
        Self { exchange_rate_map: BTreeMap::new() }
    }
}
...
impl<'a> ExchangeRateManager<'a> {
        ...
        Q: Currency,
    {
        ...
        let quote_code = quote.alphabetic_code();
        match any_true!(
            self.exchange_rate_map.contains_key(&(base_code, quote_code)),
            self.exchange_rate_map.contains_key(&(quote_code, base_code))
        ) {
        ...
        Q: Currency,
    {
        let base_code = base.alphabetic_code();
        let quote_code = quote.alphabetic_code();
        self.exchange_rate_map.remove(&(base_code, quote_code)).map(ExchangeRate::new)
    }
        ...
        C2: Currency,
    {
        self.get(&C1::default(), &C2::default()).map_or_else(
            |_| {
                self.get(&C2::default(), &C1::default())
                    .map_or(Err(ExchangeRateManagerError::ExchangeRateNotFound), |rate| {
                        Ok(rate.convert_to_base(amount))
                    })
            },
...
mod tests {
    ...
    fn test_exchange_rate_manager_operations() {
        ...
        assert_eq!(manager.insert(&gbpusd), Err(ExchangeRateManagerError::AlreadyExists));

        // Test contains_key.
        assert_eq!(manager.contains_key(&GBP::default(), &USD::default()), Some(("GBP", "USD")));
        assert_eq!(manager.contains_key(&USD::default(), &GBP::default()), Some(("GBP", "USD")));
        ...
        assert_approx_eq_money!(manager.convert(&m1, &GBP::default()).unwrap(), expected, 10e-7);

        // Test convert to quote.
        let m2: Money<GBP> = Money::new(1.0);
        let expected: Money<USD> = Money::new(1.28510);
        assert_approx_eq_money!(manager.convert(&m2, &USD::default()).unwrap(), expected, 10e-7);
